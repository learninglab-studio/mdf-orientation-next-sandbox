...
export default async function lyrics(req, res) {
    if (req.query && req.query.artist && req.query.song) {
        ...
        res.status(200).send(`<h1>result: ${result.comparative}</h1><pre>${lyrics}</pre><pre>${JSON.stringify({ artist: req.query.artist, song: req.query.song, lyrics: lyrics, sentiment: result }, null, 4)}</pre>`)
    }
